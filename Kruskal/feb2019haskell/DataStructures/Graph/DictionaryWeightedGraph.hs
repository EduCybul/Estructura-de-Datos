...
addVertex (WG a) v =  (WG (D.insert v (D.empty) a))

addEdge :: (Ord a, Show a) => WeightedGraph a w -> a -> a -> w -> WeightedGraph a w
addEdge (WG a) src dst peso 
          | not (D.isDefinedAt src a) && not (D.isDefinedAt dst a) = error "Los vertices no estan en el grafo"
          | otherwise = (WG (D.insert src dic a))
          where 
            dic = D.insert dst peso dic'
             where
             Just dic' = D.valueOf src a

edges :: (Eq a, Eq w) => WeightedGraph a w -> [WeightedEdge a w]
edges (WG a)  = [WE v1 peso v2  |  (v1, dv1) <- D.keysValues a, (v2,peso ) <- D.keysValues dv1 ]


successors :: (Ord a, Show a) => WeightedGraph a w -> a -> [(a,w)]
successors (WG a ) vertice
                    | not (D.isDefinedAt vertice a) = error "Vertice no esta en el grafo"
                    | otherwise =  suc 
            where
              suc = D.keysValues (dic')
                where
                  Just dic' =D.valueOf vertice a
