...
weights lista = aux (lista) (D.empty)
       where
        aux [] d = d
        aux (x:xs) d = aux xs (D.updateOrInsert x (+1) 1 d) --Tratamos con la funcion (+1)
{-weights [] = D.empty 
weights (x:xs) = D.insert x apariciones (weights xs)
        where 
            apariciones = case D.valueOf x (weights xs) of 
                    Nothing -> 1
                    Just c  -> c + 1  -}
...
huffmanLeaves str  = aux (D.keysAndValues (weights str)) PQ.empty
            where
                aux [] cola = cola
                aux ((x,y):xs) cola = aux xs (PQ.enqueue (WLeaf x y) cola)
{-
...
huffmanTree str 
            |length (nub str) < 2   = error "cadena con menos de dos simbolos"
            |otherwise = aux (huffmanLeaves str)
      where
          aux c
            | PQ.isEmpty (PQ.dequeue c) = PQ.first c
            | otherwise                 = aux (PQ.enqueue (merge (PQ.first c) (PQ.first c')) (PQ.dequeue c')) 
            where 
              c' = (PQ.dequeue c) 
...
joinDics dic dic' = aux keysValues dic'
      where
        keysValues = D.keysAndValues dic
        aux [] dic = dic 
        aux ((x,y):xs) dic = D.insert x y (aux xs dic) 
{-
...
prefixWith n  dic = aux n (D.keysAndValues dic)
  where
    aux n [] = D.empty
    aux n ((k,v):xs) = D.insert k (n:v) (aux n xs) 
